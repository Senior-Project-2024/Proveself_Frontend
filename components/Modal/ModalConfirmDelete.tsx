...
interface IModalConfirmDelete {
  setStateOpen : any
  yesFunction : ( id : string) => Promise<void>
  topic : string
  describe : string
  id : string
}

export default function ModalConfirmDelete({topic, describe, setStateOpen, yesFunction, id } : Readonly<IModalConfirmDelete>) {
  return (
    <Modal width="w-auto" height="h-auto" typeClose="inside">
      <div className="flex flex-col h-full items-center justify-center gap-[16px] py-[32px] px-[46px]">
        ...
        <div className="flex flex-row gap-[30px] mt-3">
          ...
          <button className="bg-red px-[24px] py-[10px] rounded-lg hover:bg-[#CE2A1E]" 
          onClick={ ()=> {
            yesFunction(id)
            setStateOpen(false)
          }}>
