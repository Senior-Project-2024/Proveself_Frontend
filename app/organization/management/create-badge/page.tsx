...
import { API_createBadge } from '@/lib/API';
import { useToast } from "@chakra-ui/react";

export default function CreateBadgeTemplete() {
  ...
  const onSubmit: SubmitHandler<badgeTemplateType> = async (data) => {
    if(file){
      ...
      try{
        const res = await API_createBadge(data, file, skillState);
        console.log(res)
        toast.update(toastIdRef.current,{
          title: 'Create badge successful',
          description: "We've create new badge successful.",
          status: 'success',
          duration: 5000,
          isClosable: true,
        });
        ...
        <form className="flex flex-col px-[84px] py-[40px] gap-[40px] shadow-thin rounded-[12px]" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex flex-row gap-[20px]">
            <button className="py-[14px] px-[20px] bg-brand-700 text-white medium20 rounded-[8px] 
              ...
              " 
              disabled={ !watch().badgeName || !watch().criteria || watch().dayExpired < 0 || watch().monthExpired < 0 || watch().yearExpired < 0 
                || !watch().description || !file?.name
              }
              type="submit"
            >
                Create
